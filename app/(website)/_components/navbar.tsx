    ...
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white">
      <div className=" flex h-16 container items-center justify-between gap-5 px-5 sm:px-8">
        ...
        <nav className="hidden items-center gap-6 lg:flex" aria-label="Main navigation">
          {primaryNavigation.map((item) => <Link key={item} href="#" className="text-[10px] font-medium text-slate-700 transition-colors hover:text-[#30277a]">{item}</Link>)}
        </nav>
        <div className="flex items-center gap-2.5 sm:gap-3">
          ...
          <Link href="#" className="bg-[#30277a] px-4 py-2 text-[10px] font-semibold text-white transition-colors hover:bg-[#21195b]">Book Now</Link>
        </div>
      </div>
    </header>
  );
